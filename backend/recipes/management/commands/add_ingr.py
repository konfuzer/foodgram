...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if exists:
            self.stderr.write(self.style.ERROR(f"Какие то ингредиенты уже загружены. прерываем"))
            return
        try:
            with open(file_path, 'r', encoding='utf-8') as file:
                ...
                ingredients = [
                    IngredientsModel(name=item["name"], measurement_unit=item["measurement_unit"])
                    for item in data
                ]
                if ingredients:
                    ...
                    self.stdout.write(self.style.WARNING("Нет ингредиентов к загрузке"))
        except Exception as e:
            self.stderr.write(self.style.ERROR(f"Ошибка импорта ингредиентов {e}"))
